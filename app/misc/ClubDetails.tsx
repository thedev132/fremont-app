    ...
    const fetchData = async () => {
      try {
        ...
        let response = await fetch(
          `https://fremont-app.vercel.app/api/orgs/${id}/`,
          {
            method: "GET",
            headers: {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: "#F9F9F9", padding: 16 }}>
      <View style={{ marginBottom: 8 }}>
        ...
        {day == null && time == null && location == null ? null : location !==
          null ? (
          <Text style={{ fontSize: 14, color: "#666" }}>
            {formatDayOfWeek(Number(day))} {time} @ {location}
          </Text>
        ) : (
          <Text style={{ fontSize: 14, color: "#666" }}>
            {formatDayOfWeek(Number(day))} {time}
          </Text>
        )}
      </View>
      <Divider />
