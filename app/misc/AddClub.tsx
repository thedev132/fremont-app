...
export default function AddClubScreen({ navigation }) {
  ...
  async function addClub(id: number, name: string, type: string) {
    try {
      ...
      await AsyncStorage.setItem(
        "myOrgs",
        JSON.stringify(myOrgs.sort(compareByName)),
      );
      ...
      const response = await fetch(
        `https://fremont-app.vercel.app/api/users/me/orgs/`,
        {
          method: "POST",
          headers: {
  ...
  const updateRemoveClub = async (id, name, type) => {
    setMyClubs((prevClubs) =>
      prevClubs.filter((club) => club.getId() != String(id)),
    );

    // Add the removed club back to the list of other clubs
    let newClub = new Organization(
      String(id),
      name,
      type,
      "fetch",
  ...
  async function removeClub(id: number, name: string, type: string) {
    try {
      ...
      let accessToken = await AsyncStorage.getItem("accessToken");
      const response = await fetch(
        `https://fremont-app.vercel.app/api/users/me/orgs/${id}/`,
        {
          method: "DELETE",
          headers: {
    ...
    const fetchData = async () => {
      try {
        ...
        if (storedMyOrgs != null) {
          ...
          let myClubs = json
            .map(
              (org: any) => new NestedOrganization(org.id, org.name, org.type),
            )
          ...
          let me = await getUserMe();
          let myOrgs = me.getOrgs().filter((club: any) => club.getType() === 3); // Filter by type 3
  ...
  const renderItem = ({ item }: { item: Organization }) => {
    ...
    return (
      ...
      >
        <TouchableOpacity
          onPress={() =>
            navigation.navigate("misc/ClubDetails", {
              name: item.getName(),
        ...
        >
          ...
          >
            ...
            <TouchableOpacity
              onPress={() =>
                addClub(parseInt(item.getId()), item.getName(), item.getType())
              }
