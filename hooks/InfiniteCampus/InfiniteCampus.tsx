...
export const useLogin = async () => {
  ...
  const response = await fetch(
    "https://fuhsd.infinitecampus.org/campus/verify.jsp",
    {
      headers: {
        accept:
      ...
      referrer:
        "https://fuhsd.infinitecampus.org/campus/portal/students/fremont.jsp?status=logoff",
  ...
  const checkLoggedIn = await fetch(
...
export const useGrades = async (data) => {
  const gradesDict: Record<
    string,
    { taskName: string; score: string | null }[]
  > = {};
  let hasGrades = false;
...
export const useSchedule = (
  ...
  for (let i = 0; i < rosterData.length; i++) {
    for (let j = 0; j < rosterData[i]["sectionPlacements"].length; j++) {
      let periodScheduleID =
        rosterData[i]["sectionPlacements"][j]["periodScheduleID"];
      let term = rosterData[i]["sectionPlacements"][j]["termName"];
      termName = term;
      let startTerm =
        rosterData[i]["sectionPlacements"][j]["term"]["startDate"];
...
export const useEntireSchedule = (data: any[]) => {
  const classes = data.map((course: any) => {
    ...
    const scheduleName =
      course.sectionPlacements?.[0]?.periodScheduleName || undefined;
