...
const calculateTimes = (classTimes) => {
  ...
  const classData = classTimes["classes"].map((cls) => {
    const [startHours, startMinutes, startSeconds] = cls["start"]
      .split(":")
      .map(Number);
    const [endHours, endMinutes, endSeconds] = cls["end"]
      .split(":")
      .map(Number);

    const classStartInSeconds =
      startHours * 3600 + startMinutes * 60 + startSeconds;
    const classEndInSeconds = endHours * 3600 + endMinutes * 60 + endSeconds;

    if (
      currentTimeInSeconds >= classStartInSeconds &&
      currentTimeInSeconds <= classEndInSeconds
    ) {
      isClassOngoing = true;
      ...
      nextClassRemainingTime = Math.min(
        nextClassRemainingTime,
        classStartInSeconds - currentTimeInSeconds,
      );
      firstClassStartsIn = Math.min(
        firstClassStartsIn,
        classStartInSeconds - currentTimeInSeconds,
      );
      if (prevClassEndInSeconds > 0) {
        breakDuration = Math.min(
          breakDuration || Infinity,
          classStartInSeconds - prevClassEndInSeconds,
        );
      }
    }
...
const ClassCountdown = ({ time, keyNumber }) => {
  ...
  const [currentClassRemainingTime, setCurrentClassRemainingTime] = useState(
    initialCalculation.currentClassRemainingTime,
  );
  const [nextClassRemainingTime, setNextClassRemainingTime] = useState(
    initialCalculation.nextClassRemainingTime,
  );
  const [breakDuration, setBreakDuration] = useState(
    initialCalculation.breakDuration,
  );
  const [totalClassDuration, setTotalClassDuration] = useState(
    initialCalculation.totalClassDuration,
  );
  const [isClassOngoing, setIsClassOngoing] = useState(
    initialCalculation.isClassOngoing,
  );
  const [afterSchool, setAfterSchool] = useState(
    initialCalculation.afterSchool,
  );
  const [firstClassStartsIn, setFirstClassStartsIn] = useState(
    initialCalculation.firstClassStartsIn,
  );
  ...
  return (
    <CountdownCircleTimer
      ...
      initialRemainingTime={
        isClassOngoing ? currentClassRemainingTime : nextClassRemainingTime
      }
      ...
      {() => (
        <Text
          style={{
            color: "#000",
            fontSize:
              afterSchool || isClassOngoing ? largeFontSize : baseFontSize,
            fontWeight: "bold",
            textAlign: "center",
